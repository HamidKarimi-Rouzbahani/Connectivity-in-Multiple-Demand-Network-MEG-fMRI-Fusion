function [done]=Step9_Decoding_specific_ROIs_f_pBlk(subject,Main_analysis_directory,Action,Trial_type,Decod_instead_of_Corr,searchlighting)
...
Regions={'Above_chance_decoding_rule_clusters_1_31_39',...
    'Above_chance_decoding_rule_clusters_29_18_33',...
    'Above_chance_decoding_stim_clusters_7_-77_17',...
    'Above_chance_decoding_stim_clusters_46_17_26'};
...
elseif Action>1
    ...
    elseif Decod_instead_of_Corr==1
        ...
        for comb=1:length(combinations)
            ...
            if searchlighting==1
                ...
                for region=1:length(Regions)
                    cfg.files.mask = {[fullfile(Main_analysis_directory, 'Results_temp_playing'),'\ROIs\',csub,'\',Regions{region},'_roi.img']};
                    RSA_results{region,comb}=decoding(cfg);
                    close all;
                end
            end
        end
    ...
    if Trial_type==1
        save(fullfile(Directory_for_working,num2str(csub,'S%03d'),'/',Beta_folder_name,'/',['RSA_',dec,SL,'.mat']),'RSA_results');
    elseif Trial_type==2
        save(fullfile(Directory_for_working,num2str(csub,'S%03d'),'/',Beta_folder_name_scnd,'/',['RSA_Cor_Incor_',dec,SL,'.mat']),'RSA_results');
    end

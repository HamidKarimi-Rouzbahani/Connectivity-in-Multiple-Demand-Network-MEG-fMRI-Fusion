...
if Trial_type==3
    load(['D:\Hamid\Postdoc\MD\MEG\Subjects_Average_',Stim_Resp,'Subjects_Average_Resp_aligned_Stm_coding_Clpsd_cue_colors_RDM_dec.mat']);
...
subplot_tmp=subplot(4,4,14);
hold(subplot_tmp,'on');
image(S_model2_cor,'Parent',subplot_tmp,'CDataMapping','scaled');
axis(subplot_tmp,'tight');
axis(subplot_tmp,'ij');
set(subplot_tmp,'CLim',scales,'DataAspectRatio',[1 1 1],'FontSize',10,'FontName','Calibri');
...
figure;
if Trial_type==3
    titles={'Cue color','Rule'};
else
    titles={'Stim periph','Stim side'};
end
c=0;
for region=regions_order_to_read
    c=c+1;
...
legend ([plots(1) plots(2)],{'Cue Color coding','Rule coding'})

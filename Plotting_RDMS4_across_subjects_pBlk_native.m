...
%     cors(1,c)=partialcorr(RDM_fMRI,RDM_model_Stim,RDM_model_Stim_periph,'type','Pearson');
%     cors(2,c)=partialcorr(RDM_fMRI,RDM_model_Rule,RDM_model_Stim_periph,'type','Pearson');
end
...
%         plot(times,smooth(Correlations_all_expt_stim_out(:,region),10),'LineWidth',3,'color',colors{c});
%         plot(times,smooth(Correlations_all_expt_rule_out(:,region),10),'LineWidth',3,'color',colors{c});
% 
...
%         % and now again with another calculation
%         rMEG_MRIall = correlate([yMEG xMRI xstim 1-xrule],'type','Pearson','method','semipartialcorr');
%         rMEG_MRIrule = correlate([yMEG xMRI 1-xrule],'type','Pearson','method','semipartialcorr');
%         rMEG_MRIstim = correlate([yMEG xMRI xstim],'type','Pearson','method','semipartialcorr');
%         CMEGMRIstim(time,region) = rMEG_MRIrule(2,1).^2-rMEG_MRIall(2,1).^2;
%         CMEGMRIrule(time,region) = rMEG_MRIstim(2,1).^2-rMEG_MRIall(2,1).^2;
    end

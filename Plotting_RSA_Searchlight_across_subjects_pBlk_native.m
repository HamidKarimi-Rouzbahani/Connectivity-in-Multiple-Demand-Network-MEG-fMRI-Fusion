...
%     for subject=subjects
    for subject=7
        ...
        csub = csub(1:4);
        load(fullfile(Directory_for_working,num2str(csub,'S%03d'),'/',Beta_folder_name,'/',['RSA_Cor_Incor_Dec_SearchLight.mat']),'RSA_results');                
        ...
        for i=1:length(combinations)
            tmp_RSA=nan(NoC,NoC,RSA_results{1, i}.n_decodings);
            tmp_RSA(combinations(i,1),combinations(i,2),:)=RSA_results{1,i}.accuracy_minus_chance.output;
        end
        ...
        for j=1:length(tmp_RSA)
            if info==1
                correlation_data(j)=corr(reshape(tmp_RSA(:,:,j),[NoC*NoC 1]),reshape(S_model_err,[NoC*NoC 1]),'type','Spearman','rows','complete');
            elseif info==2
                correlation_data(j)=corr(reshape(tmp_RSA(:,:,j),[NoC*NoC 1]),reshape(R_model_err,[NoC*NoC 1]),'type','Spearman','rows','complete');
            end
        end
        
        beta_file_temp(RSA_results{1, 1}.mask_index)=correlation_data;
...
ccc
        ...
        clearvars tmp_RSA correlation_data RSA_results beta_file_temp image_info

function [done]=Step6_Unwarp_MD_regions_f_pBlk(subject,Main_analysis_directory,Action,Trial_type)
...
% subs = res.subs;
% for sub=1
%     subs{sub}= sprintf('%s%0.3d','S', sub); %'S01'; % ID number for the subject we're going to analyse
% end
...
roidir=fullfile([Main_analysis_directory,'\DuncanOwen2000_MDs']);
...
rois = {'ACC_SMA_roi', 'left_IPS_roi', 'left_dorsal_lateral_PFc_roi', ...
    'left_ventral_lateral_PFC_roi', 'right_IPS_roi', ...
    'right_dorsal_lateral_PFc_roi', 'right_ventral_dorsal_PFc_roi'};

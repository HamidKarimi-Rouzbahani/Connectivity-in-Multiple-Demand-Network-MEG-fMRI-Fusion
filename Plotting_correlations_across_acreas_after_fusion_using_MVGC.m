...
%     indices=1:900;
% %     indices=indices([Times>0 & Times<3000]);
%     indices=indices([Times>0 & Times<2500]);
% else
%     Times=[-4000:sampling_time:995]+(sampling_time*5)./2-(smoothing_rate*5)./2;
%     indices=1:1000;
% %     indices=indices([Times>-3000 & Times<0]);    
%     indices=indices([Times>-2500 & Times<0]);    
% end
% titles={'Stim Side','Stim Periph','Rule','Resp'};
% 
% addpath('D:\Hamid\Postdoc\MD\Analyses\Results_temp_playing\mvgc_v1.0')
% plotting=0;
% significance=0.05;
% smoothing_rate2=20;
% for cases=1:4
%     if smoothing_rate2==0
%         datas=squeeze(cors_summerizedNew(:,regions_order_to_plot,cases));
%     else
%         for roi=regions_order_to_plot
%             datas(:,roi)=smooth(squeeze(cors_summerizedNew(:,roi,cases)),smoothing_rate2);
%         end
%     end
%     [signss(cases,:,:),pvals(cases,:,:)]=mvgc_Hamid(datas(indices,:)',significance,plotting);
% 
%     if plotting==0
%         subplot(2,2,cases);
...
%         ylabel('To')
%         CB=colorbar;
%         %     for i=1:size(MVGCresults,1)
%         %         for j=1:size(MVGCresults,2)
%         %             text(j-0.1,i,num2str(MVGCresults(i,j)),'FontSize',18)
%         %         end
%         %     end
%         title(titles(cases))
%         set(gca,'fontsize', 14);
%     end
% end
% save(['MVGC_',Stim_Resp,'2500.mat'],'signss','pvals')
...
for cases=1:4
    ...
    data_all=horzcat(data_all,datas);
end
datas=data_all;
ccc
[signss(cases,:,:),pvals(cases,:,:)]=mvgc_Hamid(datas(indices,:)',significance,plotting);

% save(['MVGC_All_',Stim_Resp,'2500.mat'],'signss','pvals')

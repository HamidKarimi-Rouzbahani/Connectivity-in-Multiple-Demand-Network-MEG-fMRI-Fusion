...
for Trial_type=[1]
    ...
    for region=[1:11]
        ...
        for subject=subjects
            ...
            csub = csub(1:4);
            tmp_RSA(:,:,c)=nan(NoC);
            tmp_RSA_perm(:,:,c,:)=nan(NoC,NoC,iterations);            
            if Trial_type==1
            ...
            for i=1:length(combinations)
                if isempty(RSA_results{region, i,1})
                    ...
                    tmp_RSA_perm(combinations(i,1),combinations(i,2),c,:)=nan;
                else
                    tmp_RSA(combinations(i,1),combinations(i,2),c)=RSA_results{region,i,1}.accuracy.output;
                    for itt=2:iterations+1
                        tmp_RSA_perm(combinations(i,1),combinations(i,2),c,itt-1)=RSA_results{region,i,itt}.accuracy.output;
                    end
                end
            end
            for i=1:size(tmp_RSA,1)
                for j=1:size(tmp_RSA,2)
                    if i>=j
                        tmp_RSA(i,j,c)=nan;
                        tmp_RSA_perm(i,j,c,:)=nan;
                    end
                end
            end
        end
        RSA_fMRI(:,:,region)=nanmean(tmp_RSA,3);
        RSA_fMRI_perm(:,:,region,:)=reshape(tmp_RSA_perm,[size(tmp_RSA_perm,1) size(tmp_RSA_perm,2) 30*iterations]);
%         [region]
%         ccc
%         if subject==2
%         ccc
%         end
    ...
    for region=regions_order_to_read
        ...
        for time=1:length(Times)
            ...
            for iteration=1:30*iterations
                RDM_fMRI_rand=reshape(RSA_fMRI_perm(:,:,region,iteration),[NoC*NoC 1]);
                RDM_fMRI_rand=RDM_fMRI_rand(~isnan(RDM_fMRI_rand));
                if    Trial_type==3
                    Correlations_all_out(time,region,iteration+1)=partialcorr(RDM_MEG,RDM_fMRI_rand,[RDM_model_Rule],'type','Pearson');
                    Correlations_all_expt_target_out(time,region,iteration+1)=corr(RDM_MEG,RDM_fMRI_rand,'type','Pearson');
                    fusion(time,c,iteration+1)=Correlations_all_expt_target_out(time,region,iteration+1)-Correlations_all_out(time,region,iteration+1);
                elseif Trial_type==1 || Trial_type>3
                    Correlations_all_out(time,region,iteration+1)=partialcorr(RDM_MEG,RDM_fMRI_rand,[RDM_model_Stim],'type','Pearson');
                    Correlations_all_expt_target_out(time,region,iteration+1)=corr(RDM_MEG,RDM_fMRI_rand,'type','Pearson');
                    fusion(time,c,iteration+1)=Correlations_all_expt_target_out(time,region,iteration+1)-Correlations_all_out(time,region,iteration+1);
                end
            end
    ...
    ccc
    save(['Fusion_trial_permutation_',datas{tt},'_',Stim_Resp,'_aligned.mat'],'Correlations_all_out','Correlations_all_expt_target_out','fusion')
end
ccc

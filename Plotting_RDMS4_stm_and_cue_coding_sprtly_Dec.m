...
elseif Trial_type==1 || Trial_type>3
    ...
    elseif Trial_type==10
        title('Stim side')
    elseif Trial_type==11 || Trial_type==12
        title('Response')
    end
end
...
xtickangle(45)
% ylim([-0.8 1])
ylabel('Pearson''s correlation to model(r)')
if Trial_type==3
    title('Rule')
elseif Trial_type==1
    title('Stim periph')
...
% legend (plots,titles)
xlabel('Time (ms)')
ylabel('Pearson correlation to model (r)')
box off
xlabel('Time (ms)')
...
for region=regions_order_to_read
    ...
    for time=1:length(Times)
        ...
        if    Trial_type==3
            ...
            data(time,c)=R_sqrd(time,region);
...
Correlations_all_expt_stim_out2=[Correlations_all_out(:,1) nanmean(Correlations_all_out(:,2:3),2) nanmean(Correlations_all_expt_stim_out(:,4:5),2),...
    nanmean(Correlations_all_out(:,6:7),2) nanmean(Correlations_all_out(:,8:9),2) nanmean(Correlations_all_out(:,10:11),2)];
c=0;
for region=1:size(cors_summerized,2)
    c=c+1;
    plot(Times,smooth(Correlations_all_expt_stim_out2(:,region),smoothing_rate),'LineWidth',3,'color',colors_summerized{c});
    hold on;
end
ylim([-1 1.3])
if strcmp(Stim_Resp,'Stim')
...
box off
legend(Regions_summereized,'location','southwest')
...
for region=1:size(cors_summerized,2)
    c=c+1;
    plot(Times,smooth(cors_summerized(:,region),smoothing_rate),'LineWidth',3,'color',colors_summerized{c});
    hold on;
end
ylim([-1 1.3])
if strcmp(Stim_Resp,'Stim')
...
for time=1:length(Times)
    ...
    for region=regions_order_to_read
        ...
        if Trial_type==1 || Trial_type==10 || Trial_type==11
...
%     c=c+1;
%     if Trial_type==1 || Trial_type>3
%         plot(Times,smooth(CMEGMRIstim(:,region),smoothing_rate),'LineWidth',3,'color',colors{c});
%     elseif Trial_type==3
%         plot(Times,smooth(CMEGMRIrule(:,region),smoothing_rate),'LineWidth',3,'color',colors{c});
%     end
%     hold on;
% end
% set(gca,'fontsize',18)

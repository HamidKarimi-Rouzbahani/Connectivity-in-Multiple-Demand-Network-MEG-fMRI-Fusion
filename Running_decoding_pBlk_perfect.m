...
for subject=[28:31]
    ...
    Trial_type=1; % 1=correct (for RSA=colors*rules*stimuli; for decoding= regresors are periph, rules, resp ); 2=same errors (for RSA= the same as previous);
    ...
    fmri_data_address='D:\Hamid\Postdoc\MD\fMRI_Scans';
...
%     Rgression_B_estimate_done=Step5_Beta_Weights_Estimation_f_pBlk(subject,Main_analysis_directory,Action,Trial_type);
    ...
    %
    Decoding_done=Step8_Decoding_f_pBlk3(subject,Main_analysis_directory,Action,Trial_type,Decod_instead_of_Corr,Searchlighting);
    %
